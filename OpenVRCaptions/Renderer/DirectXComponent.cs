...
namespace WpfSharpDxControl
{
	...
	public abstract class DirectXComponent : Win32HwndControl
	{
		...
		public bool OverrideRenderSize { get; set; }
		...
		protected virtual void InternalInitialize()
		{
			...
			if (OverrideRenderSize)
			{
				SurfaceWidth = (int)(OverrideWidth < 0 ? 0 : Math.Ceiling(OverrideWidth * dpiScale));
				SurfaceHeight = (int)(OverrideHeight < 0 ? 0 : Math.Ceiling(OverrideHeight * dpiScale));
			}
			else
			{
		...
		protected virtual void BeginRender()
		{
			if (OverrideRenderSize)
			{
				_device.ImmediateContext.Rasterizer.SetViewport(0, 0, (float)OverrideWidth, (float)OverrideHeight);
			}
			else
			{
